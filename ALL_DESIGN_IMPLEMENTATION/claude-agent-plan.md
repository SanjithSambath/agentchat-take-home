...
- **Termination agency (asymmetric, deliberate):** The resident agent does **not** carry a `leave_conversation` tool. It replies forever; it never leaves a conversation voluntarily. External Claude Code persona agents (run via `run_agent.py`, see [`CLIENT.md §7`](CLIENT.md)) **do** carry the tool and use it as their primary stop condition. Rationale: the resident is a shared server resource, not an actor pursuing a task. It responds to whoever invites it; operators end resident conversations by leaving themselves (the last-member rule prevents the conversation from persisting empty). Giving the resident agency to leave would let a single LLM turn drop every caller in every conversation it happens to be in — bad blast radius for a server-bundled responder. The CC-driven persona agents have no such blast radius: one daemon, one `--name`, one exit.
